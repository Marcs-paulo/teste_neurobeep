...
void BluetoothCommunication::updateTelemetryCache()
{
    ...
    *ptr++ = 0xEF;
    ...
    memcpy(ptr, &diffCar.left_velocity_ms, 4);
    ptr += 4;
    memcpy(ptr, &diffCar.right_velocity_ms, 4);
    ptr += 4;
    memcpy(ptr, &diffCar.left_velocity_target, 4);
    ptr += 4;
    memcpy(ptr, &diffCar.right_velocity_target, 4);
    ptr += 4;

    // PWM (2 x float32)
    memcpy(ptr, &diffCar.left_motor_pwm, 4);
    ptr += 4;
    memcpy(ptr, &diffCar.right_motor_pwm, 4);
    ptr += 4;

    // Gains (2 x float32)
    memcpy(ptr, &diffCar.left_gain, 4);
    ptr += 4;
    memcpy(ptr, &diffCar.right_gain, 4);
    ptr += 4;
    ...
    memcpy(ptr, &odom_vx, 4);
    ptr += 4;
    memcpy(ptr, &odom_vy, 4);
    ptr += 4;
    memcpy(ptr, &odom_omega, 4);
    ptr += 4;
    ...
    memcpy(ptr, &accel_x, 4);
    ptr += 4;
    memcpy(ptr, &accel_y, 4);
    ptr += 4;
    memcpy(ptr, &accel_z, 4);
    ptr += 4;

    // Line sensor (1 x int16 distance mm)
    int16_t line_dist = diffCar.line_distance_mm;
    memcpy(ptr, &line_dist, 2);
    ptr += 2;

    // Line markers (1 x uint16 count + 1 x float32 distance = 6 bytes)
    memcpy(ptr, &diffCar.line_marker_count, 2);
    ptr += 2;
    memcpy(ptr, &diffCar.line_marker_distance_m, 4);
    ptr += 4;

    // GPS (4 x float32 + 1 x uint8 = 17 bytes)
    memcpy(ptr, &diffCar.gps_latitude, 4);
    ptr += 4;
    memcpy(ptr, &diffCar.gps_longitude, 4);
    ptr += 4;
    memcpy(ptr, &diffCar.gps_altitude, 4);
    ptr += 4;
    memcpy(ptr, &diffCar.gps_speed, 4);
    ptr += 4;
    *ptr++ = diffCar.gps_valid ? 1 : 0;
    ...
    for (int i = 0; i < rfid_len; i++)
        *ptr++ = diffCar.rfid_uid[i];
    for (int i = rfid_len; i < 12; i++)
        *ptr++ = 0; // pad with zeros
...
void BluetoothCommunication::handler()
{
    ...
    if (bleData.length() > 0)
    {
        ...
        else if (bleData.startsWith("VEL:"))
        {
            ...
            // Opcional: Se receber VEL muito rápido (joystick), talvez não queira responder 
            // telemetria toda vez para não travar o BLE. Se quiser economizar:
            // sendTelemetryResponse = false; 
        }
        // CASO 3: Requisição Explícita ("RQS")
        else if (bleData.startsWith("RQS"))
        {
            // Não fazemos nada aqui, deixamos o código unificado lá embaixo enviar a resposta
            // para evitar duplicação de código.
            if (DEBUG_BLE) Serial.println("RQS Recebido");
        }
        // CASO 4: Genérico
        else 
        {
        ...
        // (Exceto se você setou a flag para false acima)
